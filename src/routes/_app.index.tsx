...
import { mockDashboard, mockOS } from "@/lib/mock-data";
...
function Dashboard() {
  const { metrics, porStatus, evolucao } = mockDashboard;
  const recent = mockOS.slice(0, 5);
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        <MetricCard label="OS Abertas" value={metrics.totalAbertas} icon={ClipboardList} tone="info" trend="+12% vs mês anterior" />
        <MetricCard label="Em andamento" value={metrics.emAndamento} icon={Clock} tone="default" trend="3 com prazo crítico" />
        <MetricCard label="Concluídas no mês" value={metrics.concluidasMes} icon={CheckCircle2} tone="success" trend="+8 esta semana" />
        <MetricCard label="Receita do mês" value={`R$ ${metrics.receitaMes.toLocaleString("pt-BR")}`} icon={DollarSign} tone="warning" />
      </div>
